...
class Student:
    ...
    def work(self, upens_money):
        ...
        if self.money < 50:
            ...
            print(
                f"System:  Now {self.name} has worked and collect {random_add_money}$! Now the budget {self.name}'s is {self.money}$!\n")
        if upens_money > 0:
            print(
                f"System:  Now {self.name} has worked and collect {upens_money}$!  Now the budget {self.name}'s is {self.money}$!\n")
        elif upens_money < 0:
            print(
                f"System:  Now {self.name} spent on vacation {-upens_money}$! Now the budget {self.name}'s is {self.money}$!\n")
